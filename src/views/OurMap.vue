...
const successCallback = (position: GeolocationPosition) => {
  //hardcode the position of 台大體育館
  //currentLocation.value.lat = position.coords.latitude;
  currentLocation.value.lat = 25.0217;
  //currentLocation.value.lng = position.coords.longitude;
  currentLocation.value.lng = 121.5351;

  // 使用者目前位置
  //marker.setPosition(new google.maps.LatLng(position.coords.latitude, position.coords.longitude));
  marker.setPosition(new google.maps.LatLng(currentLocation.value.lat, currentLocation.value.lng));
